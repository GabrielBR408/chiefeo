...
async function handlePost(req, res, USER_ID, USER_LABEL) {
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    // Allowed task_type vocabulary. Anything else falls back to 'action' so a
    // typo in a skill payload can't crash the row insert.
    const VALID_TASK_TYPES = ['action', 'quick_fu', 'track', 'office_only', 'delay'];
    const row = {
      user_id: USER_ID,
      name: t.name,
      ...
      // Skill-supplied operational type. Skills doing keyword detection
      // (Quick FU / Track / Office Only / Delay) pass it here; otherwise the
      // row defaults to 'action' which matches the column's DEFAULT.
      task_type: VALID_TASK_TYPES.includes(t.task_type) ? t.task_type : 'action',
    };
